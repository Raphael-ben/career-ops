...
## Machine paths for the `pro/` CV pipeline (DATA_DIR / PYTHON)

<!-- The pro/* modes (write-cv, write-cl, verify, humanize, pipeline, apply,
     tracker-entry, scan) resolve two machine-specific values from the two
     lines below. Uncomment and set them to YOUR paths.

     DATA_DIR — where your profile bank + LaTeX templates live. The profile
                bank is read from {DATA_DIR}/config/profile_bank.json.
     PYTHON   — the interpreter that has PyMuPDF + curl_cffi (used by
                cv_fitcheck.py and the Scrapling JD fetch).

     If these lines are absent, the modes default DATA_DIR to the repo's
     config/ directory and PYTHON to `python3`. Keep the `KEY: value` shape
     exactly — the modes grep for the `DATA_DIR:` / `PYTHON:` line prefix. -->

<!-- Examples (edit and uncomment):
DATA_DIR: /Users/you/path/to/job-hunter-data
PYTHON: /Users/you/path/to/.venv/bin/python3
-->
